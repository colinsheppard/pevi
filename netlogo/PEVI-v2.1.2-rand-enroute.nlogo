...
to seek-charger
  ...
  ifelse #min-taz = -99 [
    ...
    ifelse #min-taz = current-taz [
      ...
      ; This needs to be here, so it affects both possibilities of the last if statement
...
to break-up-trip
  ...
  let #available-charger-types charger-types with [ (level <= 1) or ([name] of ([this-vehicle-type] of myself) != "two-wheel") ]
    ; Randomize selection while making sure that the resulting TAZ has an existing charger
  foreach shuffle #cand-taz-list [
    let #this-dist distance-from-to [id] of current-taz [id] of ?
    if #this-dist <= remaining-range / charge-safety-factor and count chargers with [location = ?] > 1 [
      set #max-taz one-of #cand-taz-list
      set #max-dist distance-from-to [id] of current-taz [id] of #max-taz
      set #max-score 1
    ]
  ]
...
to travel-time-event-scheduler
  ...
  ifelse sum [itin-change-flag] of self > 9 [
    ...
    time:schedule-event self task arrive (ticks + time-from-to [id] of current-taz [id] of destination-taz)
  ]
end
