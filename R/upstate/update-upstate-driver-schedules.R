...
for(od.input.file in list.files(pp(pevi.shared,'data/inputs/driver-input-file/upstate-uncombined/'))){
	...
	write.table(od.table,file=pp(pevi.shared,'data/inputs/driver-input-file/upstate-uncombined/updated/',substr(od.input.file,1,nchar(od.input.file)-4),'.txt'),sep='\t',row.names=F)
}

# The frst line will need to be adjusted to remove the quotes for use in NetLogo.