...
d_ply(combined.place.data,.(county),function(df){
	if(length(na.omit(df$tripdistance))>0) {
		...
		ggsave(p,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_county/Upstate_Trip_Distance_frequency_county_',df$county[1],'.pdf'),width=17,height=11)
	}
	...
	if(length(na.omit(df$actdur))>0) {
		...
		ggsave(pa,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_county/Upstate_Activity_Duration_frequency_county_',df$county[1],'.pdf'),width=17,height=11)
	}
	if(length(na.omit(df$mode))>0) {
		...
		ggsave(pm,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_county/Upstate_Trip_Mode_frequency_county_',df$county[1],'.pdf'),width=17,height=11)
	}
		...
		ggsave(ps,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_county/Upstate_Trip_Depart_Time_frequency_county_',df$county[1],'.pdf'),width=17,height=11)
	}
})
...
d_ply(upstate.place,.(dep_hr),function(df){
	if(length(na.omit(df$tripdistance))>0) {
		...
		ggsave(p,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_hour/Upstate_Trip_Distance_frequency_hour_',df$dep_hr[1],'.pdf'),width=17,height=11)
	}
	if(length(na.omit(df$tripdur))>0) {
		pt <- ggplot(df, aes(x=tripdur)) + geom_bar(stat="bin") + ggtitle(pp("Trip duration distribution for hour ",df$dep_hr[1]))
		ggsave(pt,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_hour/Upstate_Trip_Duration_frequency_hour_',df$dep_hr[1],'.pdf'),width=17,height=11)
	}
	if(length(na.omit(df$actdur))>0) {
		pa <- ggplot(df, aes(x=actdur)) + geom_bar(stat="bin") + ggtitle(pp("Activity duration distribution for hour ",df$dep_hr[1]))
		ggsave(pa,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_hour/Upstate_Activity_Duration_frequency_hour_',df$dep_hr[1],'.pdf'),width=17,height=11)
	}
	if(length(na.omit(df$mode))>0) {
		pm <- ggplot(df, aes(x=mode)) + geom_bar(stat="bin",binwidth=1) + ggtitle(pp("Trip mode distribution for hour ",df$dep_hr[1]))	
		ggsave(pm,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_hour/Upstate_Trip_Mode_frequency_hour_',df$dep_hr[1],'.pdf'),width=17,height=11)
	}
	if(length(na.omit(df$ctfip))>0) {
		...
		ggsave(pc,filename=pp('~/Dropbox/serc/pev-shared/data/CHTS/plots_by_hour/Upstate_Trip_County_frequency_hour_',df$dep_hr[1],'.pdf'),width=17,height=11)
	}
})